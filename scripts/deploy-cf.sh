...
#
# 需要：
#   CF_API_TOKEN          Cloudflare API Token（Pages 编辑权限）
#   CLOUDFLARE_ACCOUNT_ID Cloudflare 账户 ID
#   API_KEY               tokenhub.tencentmaas.com 的 Bearer Key（也可放仓库根 .dev.vars 的 API_KEY=...）
#
# 用法：
#   CF_API_TOKEN=xxx CLOUDFLARE_ACCOUNT_ID=yyy API_KEY=zzz bash scripts/deploy-cf.sh
#
# 部署后：到 Cloudflare Pages 控制台 -> znqm-site -> Custom domains 添加 znqm.cloud（一次性）。
set -euo pipefail

: "${CF_API_TOKEN:?请提供 CF_API_TOKEN（Cloudflare API Token）}"
